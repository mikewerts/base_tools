SELECT 
    ...
    SUBSTRING(qt.text,qs.statement_start_offset/2 +1, 
	 ...
	 DB_NAME(qt.dbid) AS 'Complied Query DB Name',
	 DB_NAME(qp.dbid) as 'Ad Hoc Query DB Name',
	...
	qs.last_execution_time,
     ...
     qs.total_elapsed_time/qs.execution_count AS "Avg Duration (microseconds)",
     qp.query_plan AS "Plan"
FROM sys.dm_exec_query_stats AS qs 
CROSS APPLY sys.dm_exec_sql_text(qs.sql_handle) AS qt 
CROSS APPLY sys.dm_exec_query_plan(qs.plan_handle) AS qp
WHERE  DB_NAME(qp.dbid)  = 'DEV_Claimsxpress'
and qs.total_elapsed_time/qs.execution_count >= 1000 
--and qs.execution_count > 1

and creation_time > '4/8/2020'
