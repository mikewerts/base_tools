...
--on J.owner_sid = sj.owner_sid
LEFT JOIN
master.dbo.syslogins L
ON sj.owner_sid = L.sid

left join
msdb.dbo.syscategories sc on sc.category_id = sj.category_id
left join msdb.dbo.sysmaintplan_subplans sms on sms.job_id = SJ.job_id
...
/*
select sj.name, sc.name as 'Category Name' , sj.enabled, ss.* --enabled as 'Schedule Enabled'
from msdb.dbo.sysjobs sj
...
--where sc.name in ('Availability Group Sync','Database Maintenance')

--where sc.name = 'Database Maintenance'
--and 
--freq_type <> 1  /* 'One Time' */
--and subplan_id is null
--and SJ.name = 'DatabaseBackup - ALL_DATABASES - FULL'

--where L.name = 'MEMIC1\C2G'
--where active_start_time between '10000' and '50000'

--where SJ.name = 'Sync-DbaAvailabilityGroup'
/* weekly on Friday 

where (freq_type = 8 and freq_interval = 32 and active_start_time > 180000) 

-- weekly --
or
--where 
(freq_type = 16 and active_start_time > 180000)

-- weekday --
--where 
or (freq_type = 8 and freq_interval in(2,4,8,16,32) and active_start_time > 180000) 

-- daily --


or (freq_type = 4 and active_start_time > 180000)
