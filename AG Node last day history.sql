...
--top 1
   event_timestamp, --= 
   DATEPART(hh,event_timestamp),
   CAST(event_timestamp as int),
   --MAX(event_timestamp),
--        dateadd(hour, @utc_adjustment, event_data.value('(event/@timestamp)[1]', 'datetime')),
    ag_name, -- = 
--        event_data.value('(event/data[@name = "availability_group_name"]/value)[1]', 'varchar(64)'),
...
--        event_data.value('(event/data[@name = "previous_state"]/text)[1]', 'varchar(64)'),
    current_state,
	boot_time.create_date 'Last Reboot'
	
from curr_state

cross apply (select create_date FROM sys.databases WHERE NAME='tempdb') boot_time
where object_name = 'availability_replica_state_change'
and CAST(event_timestamp as int) = (select top 1 cast(event_timestamp as int) from curr_state  where object_name = 'availability_replica_state_change'
